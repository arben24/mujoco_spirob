...
Man kann in Matlab/Simulink Mujoco als Co simulation nutzen. Dafür muss man zu installation den installation aus folgendem git repo folgen: https://github.com/mathworks-robotics/mujoco-simulink-blockset.git  geht auch unter windows wurde aber noch nicht getestet

unter linux kann es heir zu problemen mit der pathdef.m geben. diese wie in der anleitung im system speichern: savepath ~/Documents/MATLAB/pathdef.m und wenn man dann matlab startet muss das aus diesem ordner erfolgen
startet geht unter linux mit /usr/local/MATLAB/R2025b/bin/matlab

falls skalierungsprobleme dann QT_AUTO_SCREEN_SCALE_FACTOR=0 QT_SCALE_FACTOR=2 /usr/local/MATLAB/R2025b/bin/matlab

dann kann man in matlb das repo hier als öffnen un dann in apps/Matlab. und da ist dann die simulink datei. dabei ist da beriets der Mujoco plant block drin un da muss man die .xml einfügen welche bei  
```bash
uv run apps/generate_2d_spirob.py
```
generieret wurde. 

danach kann man normal in simulink arbeiten.


MuJoCo can be used as a co-simulation partner in MATLAB/Simulink. Follow the installation instructions from the official GitHub repository: [https://github.com/mathworks-robotics/mujoco-simulink-blockset.git](https://github.com/mathworks-robotics/mujoco-simulink-blockset.git), which also works on Windows (though untested). 

On Linux, path issues with `pathdef.m` may arise—save it as instructed with `savepath ~/Documents/MATLAB/pathdef.m`, and launch MATLAB from `/usr/local/MATLAB/R2025b/bin/matlab`; for scaling problems, use `QT_AUTO_SCREEN_SCALE_FACTOR=0 QT_SCALE_FACTOR=2 /usr/local/MATLAB/R2025b/bin/matlab`. Open this repository in MATLAB, navigate to `apps/Matlab/`, and load the Simulink file, which already includes the MuJoCo Plant block—insert the `.xml` file generated by running 
```bash
uv run apps/generate_2d_spirob.py
```
You can then work normally in Simulink.
