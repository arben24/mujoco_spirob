...
class ExperimentConfig(BaseModel):
    ...
    controller_info: Optional[str] = Field(default=None, description="Controller description")
    ...
    position_estimator_segments: Optional[List[int]] = Field(default=None, description="Specific segments to estimate, None for all")
    initial_positions: Optional[Dict[int, List[float]]] = Field(default=None, description="Initial positions {seg_id: [x,y,z]}")
    initial_orientations: Optional[Dict[int, List[float]]] = Field(default=None, description="Initial orientations {seg_id: [w,x,y,z]}")

class ExperimentRecord(BaseModel):
    run_id: str = Field(..., description="Unique run identifier")
    timestamp: datetime.datetime = Field(default_factory=datetime.datetime.now, description="Creation timestamp")
    config: Optional[ExperimentConfig] = Field(default=None, description="Simulation configuration")
