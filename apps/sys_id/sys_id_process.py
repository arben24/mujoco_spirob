#!/usr/bin/env python3
"""
SpiRob – Video Processing and Synchronization

Processes a video file to extract ArUco joint markers (0-13) and sync markers (>=20).
Synchronizes the video timestamps with the recorded CSV data from `sys_id_record.py`
using the sync markers, and merges everything into a single `.parquet` file.

schau dir die vorliegende datei an. darin wird ein video mit sensor werten synchronisieret.  mach mir eine neue python datei mit der ich dann die daraus enstehendes datei bearbeiten kann. vor allem geht es mir darum am anfang und am ende des datensatze samples wegzuschneiden. achte daraf  das dann zu beispiel die globale zeit angepasst wird sodass dasn neu 0 neu gesetzt wird genbauso wie frame ID führe den code möglichst slbst auch um mögliche fehler selber zu bemerken. bedenke das uv als manager genommen wird
"""
...
class SyncArucoTracker:
    ...
    def process(self):
        ...
        if not records:
            print("Keine Gelenk-Marker gefunden!")
            return
        ...
        if not offsets:
            print("WARNUNG: Keine übereinstimmenden Sync-Marker zwischen Video und CSV gefunden.")
            offset_val = 0.0
        else:
            offset_val = float(np.median(offsets))
            print(f"Sync erfolgreich. Ermittelter Zeitversatz (CSV - Video): {offset_val:.3f} s")
...
if __name__ == "__main__":
    ...
    # Passe diese Pfade an deine realen Dateinamen an
    video_file = base_dir / "Video" / "GX010070.MP4" 
    csv_file = base_dir / "build" / "recorded_sys_id_auto_data.csv"
    output_file = base_dir / "build" / "sys_id_auto_GX010070.parquet"
