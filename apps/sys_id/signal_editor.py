"""
SpiRob Signal-Editor – Interaktive Vorverarbeitung
===================================================
Lädt eine CSV-Aufnahme (spirob_messung_*.csv), ermöglicht das manuelle
Festlegen des Schwingungsstartes und das Anwenden eines Tiefpassfilters.
Speichert das bereinigte Signal zur Auswertung mit torsion_spring_sysid.py.

Bedienung:
  Klick auf Plot       → Startpunkt direkt setzen
  Slider "Start"       → Startpunkt feinabstimmen
  Slider "Cutoff"      → Filtergrenzfrequenz anpassen
  [Filter AN/AUS]      → Tiefpassfilter ein-/ausschalten
  [◀ Vorherige]        → Vorige CSV im selben Ordner
  [Speichern]          → Bereinigte CSV als *_bearbeitet.csv speichern
  [Nächste ▶]          → Nächste CSV im selben Ordner

Verwendung:
  uv run apps/sys_id/signal_editor.py [pfad/zur/messung.csv]
  uv run apps/sys_id/signal_editor.py          # lädt neueste Messung
"""
...
# Unterstützte Signalspalten (neue zuerst, alte als Fallback)
SIGNAL_COLUMNS = ['acc_sum_YZ', 'acc_mag_YZ']
...
def load_csv(filepath: str) -> dict:
    """
    Lädt CSV-Datei und gibt dict {sensor_id: {...}} zurück.
    Unterstützt beide Spaltennamenskonventionen (acc_sum_YZ / acc_mag_YZ).
    """
    with open(filepath, 'r') as f:
        reader = csv.DictReader(f)
        fieldnames = reader.fieldnames or []

        signal_col = next((c for c in SIGNAL_COLUMNS if c in fieldnames), None)
        if signal_col is None:
            raise ValueError(
                f"Keine Signalspalte {SIGNAL_COLUMNS} in {os.path.basename(filepath)}"
            )
        ...
        for row in reader:
            ...
            raw[sid]['signal'].append(float(row[signal_col]))
...
def find_csvs_in_dir(directory: str) -> list:
    """Alle spirob_messung_*.csv im Ordner, ohne *_bearbeitet."""
...
def find_all_csvs() -> list:
    """Alle Mess-CSVs im builds-Ordner (rekursiv)."""
...
class SignalEditor:
    ...
    def _trimmed(self):
        """Gibt (t, sig_raw, sig_filt, accY, accZ) ab t_start zurück."""
    ...
    def _build_ui(self):
        ...
        # ── Oberer Plot: acc_sum_YZ ───────────────────────────────────────
        self.ax_main = self.fig.add_subplot(gs[0, :])
        self.ax_main.set_ylabel('acc_sum_YZ  (g)')
        ...
        self.ax_comp.set_xlabel('Zeit  (s)')
        self.ax_comp.set_ylabel('Beschleunigung  (g)')
        ...
        self.btn_save   = Button(self.fig.add_subplot(gs[4, 2]),
                                 'Speichern',     color='#2196F3', hovercolor='#64B5F6')
        ...
        # Klick auf Plot = Startpunkt setzen
    ...
    def _update_plot(self):
        ...
        self.info_text.set_text(
            ...
            f'Punkte: {len(t)}  │  Cutoff: {self.cutoff:.1f} Hz  │  '
            f'Filter: {"AN" if self.filter_en else "AUS"}'
        )
    ...
    def _set_span(span, x0: float, x1: float):
        """Aktualisiert ein axvspan-Rectangle in-place."""
        x1 = max(x1, x0)
        span.set_x(x0)
    ...
    def _on_toggle_filter(self, _):
        ...
        self.btn_filter.label.set_text('Filter: AN' if self.filter_en else 'Filter: AUS')
    ...
    def _on_click(self, event):
        ...
        self.slider_start.set_val(new_val)  # löst _on_start aus
    ...
    def _switch_file(self):
        ...
        self.slider_start.set_val(0.0)  # löst _on_start aus → _update_plot
        max_cutoff = min(self.fs * 0.45, 500.0)
        self.slider_cutoff.valmax = max_cutoff
        self.slider_cutoff.ax.set_xlim(1.0, max_cutoff)
        self._refresh_title()
        # Statuszeile zurücksetzen falls vorher "Gespeichert" angezeigt
        self._refresh_title()
    ...
    def _on_save(self, _):
        mask = self.t_s >= self.t_start
        _, _, sig_filt, aY, aZ = self._trimmed()
        ...
        with open(out_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['t_us', 'sensor_id', 'accY', 'accZ', 'acc_sum_YZ'])
        ...
        print(f'Gespeichert: {out_path}  ({len(t_us_trim)} Punkte, '
              f'Start={self.t_start:.4f}s, Cutoff={self.cutoff:.1f}Hz)')
        self.ax_main.set_title(
            f'✓  Gespeichert: {os.path.basename(out_path)}', color='#4CAF50', fontsize=10
        )
        self.fig.canvas.draw_idle()
...
def main():
    if len(sys.argv) > 1:
        ...
        if not os.path.isfile(path):
            ...
            sys.exit(1)
        # Lade Geschwisterdateien im selben Ordner für Navigation
        ...
        if not filepaths:
            print(f'Keine CSV-Dateien in {BUILDS_DIR} gefunden.')
            print('Verwendung: uv run apps/sys_id/signal_editor.py [pfad.csv]')
            sys.exit(1)
        idx = len(filepaths) - 1  # neueste zuerst
        print(f'{len(filepaths)} Messung(en) gefunden – starte mit der neuesten.')
